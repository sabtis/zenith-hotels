...
const AccountScreen: React.FC<AccountScreenProps> = ({
  ...
  const safeStats = {
    ...
    zenithPoints: stats?.zenithPoints ?? 0,
    memberSince: stats?.memberSince ?? '2024',
    tier: stats?.tier ?? 'Gold',
    honorProgress: stats?.honorProgress ?? 0,
    referralsCount: stats?.referralsCount ?? 0,
  };

  const menuItems = [
    { icon: Trophy, label: 'Conquistas', value: '12 Desbloqueadas', action: () => {} },
    { icon: Gift, label: 'Recompensas', value: '3 Disponíveis', action: () => {} },
    { icon: Users, label: 'Referências', value: `${safeStats.referralsCount} Convidados`, action: () => {} },
    { icon: Calendar, label: 'Histórico', value: `${safeStats.tripsCompleted} Viagens`, action: () => {} },
      ...
      <div className="p-8 pb-0">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-[10px] text-white/40 uppercase tracking-widest">Membro desde {safeStats.memberSince}</p>
          </div>
        ...
        <div className="relative p-6 rounded-3xl bg-gradient-to-br from-gold/20 via-gold/10 to-transparent border border-gold/20 overflow-hidden mb-6">
          ...
          <div className="relative">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[10px] font-black text-gold uppercase tracking-[0.3em]">{safeStats.tier}</span>
            </div>
            <div className="mb-4">
              <p className="text-3xl font-black text-white mb-1">{safeStats.zenithPoints.toLocaleString()}</p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-8">
          <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
            ...
            <p className="text-xl font-black text-white">€{safeStats.totalSaved.toLocaleString()}</p>
            <p className="text-[9px] text-white/40 uppercase tracking-widest">Total Economizado</p>
          </div>
          <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
            <Trophy size={16} className="text-gold mb-2" />
            <p className="text-xl font-black text-white">{safeStats.tripsCompleted}</p>
